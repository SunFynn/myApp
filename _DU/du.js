(function () {
  class Du {
    constructor() {
      ...
      this.langaugeObj = {
        ...
        'en-US': {
          ...
          DU_sendSeparately:
            'Send separately (receivers cannot see each other and replies are not visible)',
          DU_sendSingle: 'Send individual chat notification',
          DU_Message_timeError:
            'The scheduled DU time is in the past. Please choose a different time.',
          DU_Message_sendSuccess: 'Send successful',
          DU_Message_requestError: 'Interface error',
          DU_Message_userIdsError:
            'There are users in the current selection that do not meet the required status. Please select again.',
        ...
        'ja-JP': {
          ...
          DU_Message_timeError:
            '設定したタイマーDUが過去の時刻です。時間を再選択することをお勧めします。',
          DU_Message_sendSuccess: '送信に成功しました',
          DU_Message_requestError: 'インターフェースエラー',
          DU_Message_userIdsError:
            '現在の選択には状態に合わないユーザーが含まれています。再度選択してください',
          DU_Message_noUserIdsError: '受信者を選択してください',
        },
      };

      this.SERVERPATH = 'http://www.wtz-lmm.cn/publicShareCDN/_DU';
      ...
      // layuiCss.href = './layui/css/layui.css';
      layuiCss.href = `${this.SERVERPATH}/layui/css/layui.css`;
    ...
    async init(object, callback, closeCallback) {
      ...
      this.localParams.radio1 = object?.params?.regularTimeRadio;
      this.localParams.DuTime = object?.params?.DuTime;
      this.localParams.radio2 = object?.params?.separateRadio;
    ...
    Header_Element() {
      const DUBox_Header = this.createElement('div', {
        ...
        innerHTML: `<div class='DUBox_Header_title'>${
          this.options.title || this.langauge.DU_title
        }</div>`,
      });
        ...
        onclick: () => {
          if (!this.options.loading) {
            ...
            let obj = {
              ...
              content: DuText,
              again: tixingValue,
              timing: this.localParams.DUTime?.slice(0, -3) || '',
              read_only: Number(radio2),
              appendix: JSON.stringify(appendix),
              session: `0-${userInfo.user_id}`, // 来自会话 0-user_id  单聊  1-group_id 群聊
              create_type: 0,
    ...
    Content_Element() {
      ...
      DUBox_Content.appendChild(this.DUType_Element());

      if (this.options?.type === 0) {
        this.appDU(DUBox_Content);
      } else if (this.options?.type === 1) {
        this.shortDU(DUBox_Content);
      }
    ...
    // 类型一：应用内DU
    appDU(DUBox_Content) {
      DUBox_Content.appendChild(this.AddUser_Element());
      DUBox_Content.appendChild(this.Default_DuType_Element());
      DUBox_Content.appendChild(this.TextArea_Element());
      DUBox_Content.appendChild(this.Select_Time_Element());
      DUBox_Content.appendChild(this.Radio_Element());
    }

    // 类型二：短信DU
    shortDU(DUBox_Content) {
      DUBox_Content.appendChild(this.AddUser_Element());
      DUBox_Content.appendChild(this.Default_DuType_Element());
      DUBox_Content.appendChild(this.TextArea_Element());
      DUBox_Content.appendChild(this.Select_Time_Element());
      DUBox_Content.appendChild(this.Department_Element());
      DUBox_Content.appendChild(this.Radio_Element());
    }
    ...
    DUType_Element() {
      const duTypeBox = this.createElement('div', {
        ...
        innerHTML: `
          <div class=${this.options.type === 0 ? 'active' : ''}><span>${
          this.iconConfig.duanxinyaoqingIcon
        }</span><div></div>${this.langauge.DU_type0}</div>
          <div class=${this.options.type === 1 ? 'active' : ''}><span>${
          this.iconConfig.youxiangIcon
        }</span><div></div>${this.langauge.DU_type1}</div>
          <div><span>${this.iconConfig.dianhuaIcon}</span><div></div>${this.langauge.DU_type2}</div>
        `,
      });
      return duTypeBox;
    }
    ...
    submitHandle(data, parent) {
      console.log(data, this.options.params.userList, '====');
    ...
    Default_DuType_Element() {
      ...
      const divBox = this.createElement('div', {
        ...
        classname: 'DUBox_Content_Item_Box_Right',
        innerHTML: `
          <span class='duType_span'>${
            Number(this.options.params.organize_du || null) === 0
              ? this.langauge.DU_ordinaryDU
              : this.langauge.DU_organizeDU
          }</span>
          <span class='company_name' style='display: ${
            this.options.params.organize_du ? '' : 'none'
          }'>${this.options.userInfo.companyName || ''}</span>
    ...
    Select_Time_Element() {
      ...
      const SelectElement = () => {
        const selectDiv = this.createElement('div', {
          ...
          classname: 'DUBox_Content_Item_Box_Right',
          innerHTML: `
            <span>${this.localParams.tixingTitle}</span>
            <span class='selectIcon'>${this.iconConfig.selectXiaIcon}</span>
          `,
        });
          ...
          setTimeout(function () {
            window.layui.use(['dropdown'], () => {
              window.layui.dropdown.render({
                ...
                data: [
                  {
                    title: _that.langauge.DU_againLabel,
                    id: 0,
                    templet: `<div class=${
                      _that.localParams.tixingValue === 0 ? 'layui-select-selected' : ''
                    }>${_that.langauge.DU_againLabel}</div>`,
                  },
                  {
                    title: _that.langauge.DU_again5Label,
                    id: 5,
                    templet: `<div class=${
                      _that.localParams.tixingValue === 5 ? 'layui-select-selected' : ''
                    }>${_that.langauge.DU_again5Label}</div>`,
                  },
                  {
                    title: _that.langauge.DU_again10Label,
                    id: 10,
                    templet: `<div class=${
                      _that.localParams.tixingValue === 10 ? 'layui-select-selected' : ''
                    }>${_that.langauge.DU_again10Label}</div>`,
                  },
                  {
                    title: _that.langauge.DU_again15Label,
                    id: 15,
                    templet: `<div class=${
                      _that.localParams.tixingValue === 15 ? 'layui-select-selected' : ''
                    }>${_that.langauge.DU_again15Label}</div>`,
                  },
                ],
                click: function (obj) {
    ...
    Radio1_Box(parent) {
      ...
      if (BInput) {
        this.localParams.DuTime = undefined;
          ...
          setTimeout(() => {
            window.layui.use('laydate', () => {
              ...
              laydate.render({
                elem: '#DateTime', //指定元素
                type: 'datetime',
                value: this.localParams.DuTime || this.getTime(),
                theme: 'default',
                lang: this.options.langauge,
              });
            });
          }, 100);
        }
  ...
  Du.prototype.request = async function (cmd_id, params) {
    const URLS = {
      prod: 'http://www.wtz-lmm.cn',
      beta: 'http://www.wtz-lmm.cn',
    };
    ...
    const url = `${URLS[this.options.env]}/api/v1/messages`;
