...
export default class Waterfall {
  ...
  init() {
    if (!this.container) {
      throw new Error(`container element id:${this.config.el} is not exist`);
    }
  ...
  initLayout() {
    ...
    for (let i = 0; i < this.liNodes.length; i++) {
      const h = this.liNodes[i].offsetHeight + (this.config.rowGap || 0);
      this.itemList.push({
        index: i,
        bottom: h,
        height: h,
    ...
    for (let i = 0; i < this.liNodes.length; i++) {
      ...
      for (let j = 0; j < colList.length; j++) {
        ...
        const lastItemBottom =
          colList[j]?.[curColLastIndex]?.bottom || this.lastItemsBottom[j] || 0;
        // 最新的待塞进view的li元素
